...
20190731-new    --------------------------
"""
def get_hist_capital_flow(code=None, retry_count=3,pause=0.001):
    """
    ...
    """
    if code[:1] in ['5', '6', '9'] :
        symbol =  code + '1'  # symbol :'600xxxx1'
    else :
        symbol = code + '2'

    url = ''

    url = ct.EM_HCF_URL%(ct.P_TYPE['http'],ct.DOMAINS['em'],symbol)
...
def get_nf_realtime(code=None, retry_count=1, pause=0.001):
    """
            北向资金增持行业板块排行-北上资金板块总体情况   （eastmoney.com） http://data.eastmoney.com/hsgtcg/
        Parameters
        ------
          retry_count : int, 默认 3
                     如遇网络等问题重复执行的次数
...
def get_nbfbk_hist_capital_flow(code=None, retry_count=3,pause=0.001):
    ...
    url = ct.EM_NBFBKCF_URL%(ct.P_TYPE['http'],ct.DOMAINS['em'],code)
    #url = "http://dcfm.eastmoney.com/EM_MutiSvcExpandInterface/api/js/get?type=HSGT20_HYTJ_LS_MX&token=894050c76af8597a853f5b408b759f5d&st=HdDate&sr=-1&p=1&ps=50&js=var%20kYbJeWxa={+pages:(tp),data:(x)}&filter=(ORIGINALCODE=" + code + " )"
...
def get_nbfbk_hist_capital_flow_CSV(code=None, retry_count=3,pause=0.001):
    ...
    # code=bk_code_no
    file_name=str(code) + '_nfbk.csv'
    # 获取csv最后日期
    # df_csv = pd.read_csv('C:\\Users\\DELL\\Desktop\\ST_Web_App\\Data\\沪深股通持股记录多日.csv')
    df_csv = pd.read_csv('C:\\Users\\DELL\\Desktop\\ST_Web_App\\Data\\Industry_data\\' + file_name ,encoding='GB2312')

    print(df_csv.iloc[-1, 1])
    if df_csv.iloc[-1, 1]=='nan':
       df_csv=df_csv.drop(-1, axis=0)

    date_csv = df_csv.iloc[-1, 0]
    # date_csv=str(date_csv)


    # date_csv = df_csv.iloc[2, 0]
    date_csv = dtdt.strptime(date_csv, '%Y/%m/%d')
    # date_csv = dt.strptime(date_csv, '%Y/%m/%d')
    # print(date_csv)

    df_realtime=get_nbfbk_hist_capital_flow(code=code, retry_count=3,pause=0.001)
    # print(df_realtime.iloc[0, 0])

    if df_realtime.iloc[0, 0]==None:
       df_realtime=df_realtime.drop(0, axis=0)
    # print(df_realtime)
    # print(date_csv)

    Date_df_realtime = df_realtime.iloc[0, 0]
    # print(Date_Shareholding)
    Date_df_realtime = dtdt.strptime(str.strip(Date_df_realtime), '%Y-%m-%d')
    # Date_df_realtime = dtdt.strptime(str.strip(Date_df_realtime), '%Y-%m-%d').strftime('%Y/%m/%d')
    # print(Date_Shareholding)


    # df=df_realtime.iloc[df_realtime['date']==date_csv]


    if date_csv == Date_df_realtime:
        print('Get data from CSV')
        return df_csv

    elif date_csv < Date_df_realtime:
        ...
        while date_begin <= Date_df_realtime:
        ...
        # dates.reverse()
        df_add = df_realtime.loc[df_realtime['date'].isin(dates)]

        # Date_df_realtime = dtdt.strptime(str.strip(Date_df_realtime), '%Y-%m-%d').strftime('%Y/%m/%d')
        df_add['date'] = df_add['date'].map(lambda x: x.replace('-', '/'))
        # df_add['date']=str.replace('-','/')

        df_add=df_add.iloc[::-1]

        df_add.to_csv('C:\\Users\\DELL\\Desktop\\ST_Web_App\\Data\\Industry_data\\' + file_name , mode='a', header=False , index=False,encoding='GB2312',)

        df = pd.read_csv('C:\\Users\\DELL\\Desktop\\ST_Web_App\\Data\\Industry_data\\' + file_name , encoding='GB2312')
                # df = df.sort_index(ascending=False)
                # df = df.sort_index(ascending=True)
        d
...
def update_nbfbk_hist_capital_flow_CSV(code=None, retry_count=3,pause=0.001):
    ...
    if date_csv == Date_df_realtime:
        ...
        df_add.to_csv('C:\\Users\\DELL\\Desktop\\ST_Web_App\\Data\\Industry_data\\' + file_name, mode='a', header=False,
                      index=False, encoding='GB2312', )

        # df = pd.read_csv('C:\\Users\\DELL\\Desktop\\ST_Web_App\\Data\\Industry_data\\' + file_name)
                # df = df.sort_index(ascending=False)
                # df = df.sort_index(ascending=True)
        print('data updated !')
        # return df
